...
export const PYTH_SOL_USD_FEED = (SOLANA_NETWORK === 'mainnet-beta'
    ? 'H6ARLBq23nSua883yWuK8wg9bNisJmeFuSSvA7N2m3e4' // Mainnet
    : 'J83w4f9xNee2gTYas76Ag6N6K4X2f1E6f4SFFfM3A9'  // Devnet
).trim();
 ...
 */
export async function getSolPriceFromJupiter(simulateFailure: boolean = false): Promise<number | null> {
    if (simulateFailure) {
    ...
    try {
        const response = await fetch(`https://api.jup.ag/price/v3?ids=${JUPITER_SOL_MINT}`);
        if (!response.ok) {
            console.warn(`[Oracle Jupiter] Erro na requisição: ${response.statusText}`);
            return null;
        }
        ...
        if (isNaN(price) || price <= 0) {
            console.warn(`[Oracle Jupiter] Preço inválido obtido: ${priceData.usdPrice}`);
            return null;
        }
 ...
 */
export async function getSolPriceFromPyth(connection: Connection, simulateFailure: boolean = false): Promise<number | null> {
    if (simulateFailure) {
    ...
    try {
        console.log(`[Oracle Pyth] Resolving PublicKey for feed: ${JSON.stringify(PYTH_SOL_USD_FEED)} | Length: ${PYTH_SOL_USD_FEED.length}`);
        
        let pythPublicKey: PublicKey;
        try {
            pythPublicKey = new PublicKey(PYTH_SOL_USD_FEED);
        } catch (e) {
            console.warn(`[Oracle Pyth] Chave do feed Pyth malformada ou inválida (${PYTH_SOL_USD_FEED}). Ignorando feed on-chain.`);
            return null;
        }
        ...
        if (!accountInfo) {
            console.warn(`[Oracle Pyth] Não foi possível obter o AccountInfo para ${PYTH_SOL_USD_FEED}`);
            return null;
        }
        ...
        console.error(`[Oracle Pyth] Falha ao decodificar dados do Pyth Network:`, error);
        return null;
    }
}
 ...
 */
export async function getSolPrice(connection: Connection, simulateFailure: boolean = false): Promise<number> {
    ...
    console.log(`⏱️ [Latency Benchmark] Tempo total de consulta dual (Jup+Pyth): ${(durationMs / 1000).toFixed(3)}s`);
    ...
    if (jupPrice !== null && pythPrice !== null) {
        ...
        console.log(`[Oracle Dual] Jupiter: $${jupPrice.toFixed(4)} | Pyth: $${pythPrice.toFixed(4)} | Variância: ${variancePct.toFixed(2)}%`);
        ...
        if (variancePct > MAX_VARIANCE_PCT) {
            ...
            console.warn(`[Oracle Dual] ALERTA: Variância de preço (${variancePct.toFixed(2)}%) excedeu o limite máximo de ${MAX_VARIANCE_PCT}%. Usando Pyth por segurança.`);
            return pythPrice;
        }
    ...
    if (jupPrice !== null && pythPrice === null) {
        console.warn(`[Oracle Dual] Pyth falhou. Usando Jupiter como fallback.`);
        return jupPrice;
    }

    // Scenario 4: Both failed, raise error / fallback to historical average
    console.error(`[Oracle Dual] Crítico: Ambas as fontes de preço (Jupiter + Pyth) falharam!`);
    throw new Error("Não foi possível obter a cotação do SOL das fontes descentralizadas.");
}
